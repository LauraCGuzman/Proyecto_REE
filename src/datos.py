...
def descargar_demanda_horaria(inicio: pd.Timestamp, fin: pd.Timestamp) -> pd.DataFrame:
    ...
    semiabierta [inicio, fin), ambos tz-aware) y la agrega a horario. No
    persiste nada a disco: eso es responsabilidad de quien llame
    (`pipeline/predecir.py`, Fase 2)."""
    token = obtener_token()
    sesion = crear_sesion(token)
    df = descargar_indicador(
        sesion, INDICADOR_DEMANDA_REAL, inicio, fin, "demanda_real"
    )
    return resample_horario(df)
